...
def test_config_manager_loads_defaults():
    ...
    assert cfg.trading.max_position_size == float(
        os.environ["TRADING__MAX_POSITION_SIZE"]
    )
    assert (
        cfg.slippage_protection.max_slippage_bps
        == int(os.environ["SLIPPAGE_PROTECTION__MAX_SLIPPAGE_BPS"])
    )


def test_config_reload(monkeypatch):
    monkeypatch.setenv("PROFIT_THRESHOLD", "2.5")
    monkeypatch.setenv("SLIPPAGE_PROTECTION__MAX_SLIPPAGE_BPS", "75")
